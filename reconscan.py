...
def nmapScan(ip_address):
   ...
   serv_dict = {}
   TCPSCAN = "nmap -vv -Pn -A -sC -sS -T 4 -p- -oN '/root/Documents/EnumScripts/%s.nmap' -oX '/root/Documents/EnumScripts/%s_nmap_scan_import.xml' %s"  % (ip_address, ip_address, ip_address)
   UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oN '/root/Documents/EnumScripts/%sU.nmap' -oX '/root/Documents/EnumScripts/%sU_nmap_scan_import.xml' %s" % (ip_address, ip_address, ip_address)
...
if __name__=='__main__':
   f = open('/root/Documents/EnumScripts/targets.txt', 'r') # CHANGE THIS!! grab the alive hosts from the discovery scan for enum
   for scanip in f:
       jobs = []
